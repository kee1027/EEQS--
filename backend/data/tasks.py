...
def raster_sync_task(self, dataset_id, days_back=16, triggered_by="schedule"):
    """
    Celery 异步任务：MODIS 积雪数据每日同步（下载 -> 渲染瓦片 -> 台账）。

    Args:
        dataset_id: RasterDataset 主键
        days_back: 以今天为锚回溯天数（默认 16 天）
        triggered_by: 触发方式（schedule / manual）

    Returns:
        dict: sync_latest 的摘要 {downloaded, skipped, failed, pending, ...}
    """
    from .raster_pipeline.jobs import sync_latest

    print(f"[Celery] 开始执行栅格同步：dataset_id={dataset_id}, days_back={days_back}")
    ...
    try:
        ...
        print(f"[Celery] 栅格同步软超时：dataset_id={dataset_id}")
        raise

    print(f"[Celery] 栅格同步完成：{summary}")
    if summary.get("failed"):
        raise self.retry(exc=Exception(f"存在失败日期: {summary['failed']}"))
    return summary
