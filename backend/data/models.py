...
class RasterDataset(models.Model):
    """栅格数据集配置（如 MODIS MOD10A1 积雪，阿勒泰区域）。"""

    name = models.CharField(max_length=100, verbose_name="数据集名称")
    product = models.CharField(max_length=20, default="MOD10A1", verbose_name="产品短名")
    concept_id = models.CharField(
        max_length=50, default="C2565093311-NSIDC_CPRD", verbose_name="CMR Concept ID"
    )
    ...
    tile_dataset_name = models.CharField(
        max_length=50,
        default="modis-snow",
        verbose_name="瓦片数据集名",
        help_text="对应 settings.TILE_DATASETS 的 key",
    )
    ...
    @property
    def bbox(self):
        """earthaccess bounding_box 顺序：(min_lon, min_lat, max_lon, max_lat)"""
        return (self.min_lon, self.min_lat, self.max_lon, self.max_lat)


class RasterJob(models.Model):
    """栅格处理任务台账：每日 下载 -> 渲染瓦片 的状态机记录。"""
    ...
    triggered_by = models.CharField(max_length=50, default="manual", verbose_name="触发方式")
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="创建时间")
    ...
    def __str__(self):
        return f"{self.dataset.name} {self.date} [{self.status}]"
